...
- **25.04.04**: 对hook有了更深入的了解，本库应该编译为静态库更合适。发现多个线程的调度器唤醒同一个fd事件时可能发生竞争，添加原子量，保证每次只会有一个线程处理fd的事件。
- **25.04.06**：发现并修复在等待一个fd可读后立刻等待这个fd可写（或者顺序相反）会导致阻塞的bug。
...
### 2025.4.6
- 新增了一个测例，发现服务器建立连接后，调用recv成功后立刻调用send，会陷入阻塞。排查后发现，awake_epoll中，如果一次唤醒后导致等待队列为空，可能需要等到下一次唤醒才会删除对应fd的监听。但是在添加epoll的逻辑中，如果等待队列为空，则会认为这个fd没有被监听，会直接调用CTL_ADD进行添加，从而导致添加失败。在awake_epoll，判断如果等待队列为空，则直接删除对应fd的监听而非等到下一轮，问题解决。
